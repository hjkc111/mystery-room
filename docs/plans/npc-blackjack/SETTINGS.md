...
2026-09-12 实施更新：用户已批准实施。新建房间保存 rulesVersion=npc-cards-v1、autonomousNpc（创建页可关闭）、minigamesEnabled=true；已有房间没有此快照则保持手动模式。服务端 Secret 与本地 .env 保持原值。下文原“拟新增”表是设计基线，具体落地差异见本节。

实际：新增0004迁移（simulation、activities）；NPC 120px/s、前台2秒tick、25秒租约、最多补进2.5秒移动；自主预算24/房间、8/角色、总预算剩余24时停用自主模型；模型只用于阶段发言/投票与真人对话，导航和合法行动由程序执行。私聊预订30秒，牌桌等待30秒、每轮45秒、揭牌6秒、断线20秒。三小局、无下注。减少动态偏好已落地；本版无声音资源，保持静音。
...
6. 本轮不申请新密钥、不改变额度、不新增第三方账号。用户已批准实施和发布，使用同一仓库与公开站点；发布记录以STATUS为准。
