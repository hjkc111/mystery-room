...
开局后，在公聊输入 `@角色名 问题`（每次一位），或选择该 AI 私聊。也可点击角色卡的 @ 按钮。房主点击「让 AI 完成本幕」后，系统按规则安排该角色的调查、出示证据和准备，大模型负责角色发言及秘密指控。每幕需手动触发，房主仍控制切幕。
...
AI 玩家与主持共用服务端模型配置、密钥及每局 80 次调用预算（失败调用也计数）。新增权限、私聊隔离、失败重试、重复请求和两人两 AI 完整流程测试；本地全部 13 项通过。真实 DeepSeek 的 @ 对话、私聊和读本行动已通过；完整真人盲测尚未进行。公开站点需发布新版后生效。
