...
function floor(scene){
 ...
 for(const [x,y,w,h] of scene.walls){
  ...
  else if(scene.kind==='hall'||scene.kind==='meeting'){ctx.fillStyle='#493624';ctx.fillRect(x+12,y+8,w-24,h);ctx.fillStyle='#755336';ctx.fillRect(x,y,w,h-6);ctx.fillStyle='#bc9360';ctx.fillRect(x+8,y+8,w-16,h-22);ctx.fillStyle='#a37d50';for(let dy=24;dy<h-20;dy+=24)ctx.fillRect(x+8,y+dy,w-16,2);for(let i=32;i<w;i+=64){tile(15,2,x+i,y-32);tile(15,2,x+i,y+h);}}
...
function draw(now){requestAnimationFrame(draw);const dt=lastFrame?Math.min((now-lastFrame)/1000,.05):0;lastFrame=now;if(!state||!local)return;
 ...
 ctx.imageSmoothingEnabled=false;floor(SCENES[local.scene]);
 ...
 for(const t of targets){ctx.globalAlpha=t.done?.35:1;ctx.fillStyle=t.personal?'#dfb6ec':'#e2bd67';ctx.beginPath();ctx.arc(t.x,t.y,19+Math.sin(now/400)*2,0,Math.PI*2);ctx.globalAlpha*=.2;ctx.fill();ctx.globalAlpha=t.done?.35:1;tile(42,15,t.x-12,t.y-12,24,24);ctx.globalAlpha=1;}
 ...
 const labels=[];for(const p of people){const person=state.players.find(q=>q.id===p.id),role=person.role??0;let rendered=peers.get('render-'+p.id)||{x:p.x,y:p.y};if(distance(rendered,p)>200)rendered={x:p.x,y:p.y};else {rendered.x+=(p.x-rendered.x)*Math.min(1,dt*14);rendered.y+=(p.y-rendered.y)*Math.min(1,dt*14);}peers.set('render-'+p.id,rendered);
  ...
  ctx.fillStyle='#0006';ctx.beginPath();ctx.ellipse(x,y+10,13,5,0,0,Math.PI*2);ctx.fill();tile(role%2,role<2?6:5,x-16,y-25,32,32,characters);
  ctx.font='bold 13px system-ui';ctx.textAlign='center';const label=person.name+(person.bot?' · AI':p.id===state.me.id?' · 你':'');const width=ctx.measureText(label).width,lx=Math.max(width/2+6,Math.min(WIDTH-width/2-6,x));let ly=y-32;while(labels.some(q=>Math.abs(q.x-lx)<(q.width+width)/2+10&&Math.abs(q.y-ly)<21))ly-=22;labels.push({x:lx,y:ly,width});ctx.fillStyle='#101a1de6';ctx.fillRect(lx-width/2-5,ly-15,width+10,20);ctx.fillStyle=colors[role];ctx.fillText(label,lx,ly);
 }
 const candidates=[...doors.map(d=>({...d,kind:'door',available:true})),...targets].filter(t=>distance(local,t)<=REACH).sort((a,b)=>distance(local,a)-distance(local,b));interaction=candidates[0]||null;
 const hint=state.paused?'游戏暂停中':interaction?interaction.kind==='door'?`空格 · 进入${SCENES[interaction.to].name}`:`${interaction.done?'已调查':interaction.available?'空格 · 调查':'暂不可调查'} ${interaction.title}`:'WASD / 方向键移动 · 走近门或线索后按空格';
 if($('interactionHint').textContent!==hint)$('interactionHint').textContent=hint;$('interact').disabled=!interaction||state.paused;
 canvas.dataset.scene=local.scene;canvas.dataset.x=Math.round(local.x);canvas.dataset.y=Math.round(local.y);canvas.dataset.peers=JSON.stringify(people.map(p=>({id:p.id,x:Math.round(p.x),y:Math.round(p.y)})));
}
