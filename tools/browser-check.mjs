...
try {
 ...
 for(let i=0;i<4;i++){await pages[i].locator('#roles article').nth(i).locator('button').click();await idle();}
 async function next(){for(const p of pages){await p.locator('#ready').click();await idle();}await pages[0].locator('#advance').click();await idle();}
 ...
 for(const p of pages){await p.locator('#targets article').first().locator('button').click();await idle();assert.equal(await p.locator('#clues article').count(),1);}
 await next();for(const p of pages){assert.equal(await p.locator('#chapters article').count(),2);await p.locator('#clues button').click();await idle();}
 await next();for(let i=0;i<4;i++){await pages[i].locator('#targets article').nth(i).locator('button').click();await idle();}
