...
test('AI players: D1 permissions, mentions, private scope, failure, replay and full game',async()=>{
 ...
 const {mf}=await runtime({bindings:{DEEPSEEK_API_KEY:'test-only',AI_BASE_URL:`http://127.0.0.1:${server.address().port}`,AI_TIMEOUT_MS:'3000'}});
 ...
 async function ok(u,type,payload,requestId){const r=await action(u,type,payload,requestId);assert.equal(r.status,200,JSON.stringify(r.data));assert.equal(r.data.botError,undefined,r.data.botError);return r.data;}
 try{
  ...
  const retryId=crypto.randomUUID();await ok(users[0],'chat',{text:'PRIVATE_A',to:bots[0].id},retryId);
  const count=requests.length;await ok(users[0],'chat',{text:'PRIVATE_A',to:bots[0].id},retryId);assert.equal(requests.length,count);
  assert.ok((await state()).messages.some(m=>m.from===bots[0].id&&m.to===users[0].id));
  assert.ok(!(await state(users[1])).messages.some(m=>m.to===users[0].id||m.text==='PRIVATE_A'));
  await ok(users[1],'chat',{text:'@'+bots[0].name+' 你好'});
  assert.ok(!JSON.stringify(requests.at(-1)).includes('PRIVATE_A'));
